...
public class EqualSidesOfAnArray {
    ...
    public static int findEvenIndex(int[] arr) {
        ...
        //warunek brzegowy, gdzie index jest pierwszym elementem tablicy -- checked!
        if ((list.stream().mapToInt(x -> x).sum() - arr[0]) == 0) return 0;
        ...
        for (int i = 1; i<= arr.length-1; i++) {
            int leftSum = list.subList(0, i)
                    ...
                    .mapToInt(x -> x)
                    .sum();
            int rightSum = list.subList(i + 1, arr.length)
                    .stream()
                    .mapToInt(x -> x)
                    .sum();
